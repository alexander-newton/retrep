...
def run_replications(DIR):
    ...
    all_results_df.to_csv('./trial_results.csv')
...
def replicate_full(X,y, z, metadata):
    ...
    estimator = DoublyRobustElasticityEstimator(endog=y, exog=X, interest=interest, endog_x=endog_x, instruments=z, fe=fe, estimator_type='nn', nn_params=nnparams, elasticity=elasticity)
    fit = estimator.fit(method='ols', compute_asymptotic_variance=False, weights=weights)
    print(fit.summary())
