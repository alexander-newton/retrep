...
inter_df = year_d.mul(df['aes_1961'].values, axis=0)

# -------------------------------
# Design matrices (y in LEVELS since replicate() logs internally)
# -------------------------------
y = np.exp(df['lnpop'].astype(float).values)
...
metadata = {
    ...
    'comments': (
        'Table 4, Panel B, Column (2): lnpop on predyield1aes with '
...
replicate(
    metadata=metadata,
    y=y,                         # IMPORTANT: levels; replicate() logs internally
    X=X,
    interest='predyield1aes',
    elasticity=False,            # semi-log DV
    fe=fe_vars,
