...
lipm = macropc[:, 0]  # Industrial production (best approximation)
ls_level1 = mat_data['ls_level1'].flatten()  # v1
ls_cme_rvar = mat_data['ls_cme_rvar'].flatten()  # rv

# Create dependent variable: y=100*[NaN;diff(lipm)]
y = np.full(len(lipm), np.nan)
y[1:] = 100 * np.diff(lipm)
...
metadata = {
    ...
    'comments': 'Table 1 Column 4: Industrial Production on volatility lags'
}
...
print("Expected: v1 ≈ 0.16, rv ≈ -0.23")
