...
# =====================================
# Bilal & Kanzig (2026) - Panel Local Projections, horizon 0
# s03S_lp_panel_globalshock.do, first block
# DV: GDP_{t+0} / GDP_{t-1} (level ratio from raw GDP per capita)
# Shock: global temperature shock (Berkeley Earth, Hamilton filtered h=2, standardized)
# xtscc d{0}lnrgdppc_pwt L(0/2).shock L(1/2).dlnrgdppc_pwt
...
y_h0 = pd.DataFrame(reg_df['y_level'].values, columns=['rgdppc_ratio'])

# X matrix: interest variable first, then other regressors, then FE
X_h0 = reg_df[x_cols].reset_index(drop=True)

# Save to parquet
out_dir = os.path.join(output_folder, '179', 'PanelLP_GDP_h0')
os.makedirs(out_dir, exist_ok=True)

y_h0.to_parquet(os.path.join(out_dir, 'y.parquet'), index=False)
X_h0.to_parquet(os.path.join(out_dir, 'X.parquet'), index=False)

# Save metadata
with open(os.path.join(out_dir, 'metadata.json'), 'w') as f:
    json.dump(metadata_h0, f, indent=2)
...
replicate(
    ...
    y=y_h0.values,
    X=X_h0[x_regressor_cols].values,
    interest=f'{shock_var}_L0',
    endog_x=None,
    z=None,
    fe=X_h0[['country_code_en']].values,
    elasticity=True,
    replicated=True,
