...
# =====================================
# Bilal & Kanzig (2026) - Panel Local Projections, horizon 5
# s03S_lp_panel_globalshock.do, first block
# DV: GDP_{t+5} / GDP_{t-1} (level ratio from raw GDP per capita)
# Shock: global temperature shock (Berkeley Earth, Hamilton filtered h=2, standardized)
# xtscc d{5}lnrgdppc_pwt L(0/2).shock L(1/2).dlnrgdppc_pwt
...
y_h5 = pd.DataFrame(reg_df['y_level'].values, columns=['rgdppc_ratio'])

# X matrix: interest variable first, then other regressors, then FE
X_h5 = reg_df[x_cols].reset_index(drop=True)

# Save to parquet
out_dir = os.path.join(output_folder, '179', 'PanelLP_GDP_h5')
os.makedirs(out_dir, exist_ok=True)

y_h5.to_parquet(os.path.join(out_dir, 'y.parquet'), index=False)
X_h5.to_parquet(os.path.join(out_dir, 'X.parquet'), index=False)

# Save metadata
with open(os.path.join(out_dir, 'metadata.json'), 'w') as f:
    json.dump(metadata_h5, f, indent=2)
...
replicate(
    metadata=metadata_h5,
    y=y_h5.values,
    X=X_h5[x_regressor_cols].values,
    interest=f'{shock_var}_L0',
    endog_x=None,
    z=None,
    fe=X_h5[['country_code_en']].values,
    elasticity=True,
    replicated=True,
