"""
Table 4: Beta bond returns on stock returns - Period 1
"""
...
# Period 1: q>=78 & q<165
df_period1 = df[(df['q'] >= 78) & (df['q'] < 165)].dropna(subset=['xr_bond', 'xr_equity'])

metadata = {
    'paper_id': '080',
    'table_id': '4',
    'panel_identifier': 'beta_bond_p1',
    'model_type': 'log-linear',  # log(y) on X (where X is already log returns)
    'comments': 'Beta bond on stock, Period 1 (79:Q3-01:Q1). Bond returns in levels (logged internally), equity returns already in logs.'
}

replicate(
    metadata=metadata,
    y=df_period1['xr_bond'],  # In levels (will be logged internally)
    X=sm.add_constant(df_period1[['xr_equity']], prepend=False),  # Already in log returns
    interest='xr_equity',
