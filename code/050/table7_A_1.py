...
# Table 7 Panel A Column 1: Reduced Form
# Using AgGDPcapita in levels
regression_vars = ['AgGDPcapita', 'did'] + all_controls + ['regency63n', 'isleID']
df_clean = df[regression_vars].dropna()

y = df_clean['AgGDPcapita'].values  # Using levels
X = df_clean[['did'] + all_controls + ['isleID']]
X = sm.add_constant(X)
...
metadata = {
    ...
    'comments': 'Table 7 Panel A Column 1: Reduced form - Log Agricultural GDP per capita on expropriation intensity'
}
...
replicate(
    ...
    interest='did',  # Pass column name, not index
    elasticity=False,
    fe=['isleID'],
