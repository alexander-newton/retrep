...
# Create productivity in levels from the logged variable
df['prodnorm'] = np.exp(df['wtlnprodnorm'])

# Table 7 Panel C Column 2: IV
# Include lwaqfland_ha in X matrix along with controls
regression_vars = ['prodnorm', 'lwaqfland_ha', 'did'] + all_controls + ['regency63n', 'isleID']
df_clean = df[regression_vars].dropna()

y = df_clean['prodnorm'].values
...
metadata = {
    ...
    'comments': 'Table 7 Panel C Column 2: IV - Log Agricultural Productivity on waqf land, instrumented by expropriation intensity'
}
