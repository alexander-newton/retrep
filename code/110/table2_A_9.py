...
# =====================================
# Table 2 Panel A Column 9: All Conflict - Duration - Full Controls
# Dependent variable: ln_cfatm = ln(1 + Months of Deadly Conflict)
...
# Specification: reg ln_cfatm SL i.ccode ln_shape_area ln_dist_kilometers meanalt SI abs_lat lon split10pc sc_mean v33 v30 patrilineal, r
# Expected results: SL coeff ≈ 0.869 (0.195), v33 coeff ≈ -0.0133 (0.00964)
# =====================================
...
required_vars = ['ln_cfatm', 'SL', 'ccode'] + geo_controls + hist_controls
...
# Dependent variable: ln_cfatm (already logged)
# Convert back to levels since replicate function logs internally
y = np.exp(df_clean['ln_cfatm'])
...
metadata = {
    ...
    'comments': 'Table 2 Panel A Column 9: Months of deadly conflict on slave trade (SL) with country FE, geographic controls, and historical controls'
}
...
replicate(
    ...
    y=y,
    X=X,
    interest='SL',  # Variables of interest (as per Stata keep(SL v33))
