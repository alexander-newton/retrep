...
# --- DV: IHS(hourly wage, unconditional) ---
dv_ihs = 'el_emp_wage_all_uncd_i'
...
if missing:
    raise ValueError(f"Missing columns for Table 1 Col(4): {missing}")
...
# X build: treatments + covs, then single FE code, then constant
...
metadata = {
    ...
    'comments': 'Table 1, Col (4): IHS(hourly wage, unconditional) on public/private/placebo + baseline covariates + bl_block FE.'
}
...
replicate(
    ...
    interest=interest,
    fe=fe_indices,
    elasticity=False,     # DV already IHS; θ=1 (no scaling)
