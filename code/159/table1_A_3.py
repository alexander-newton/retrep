...
# ---------- Columns for Column (3), Panel A ----------
# Task displacement (Panel A)
tdvar = 'tdg_ro33_bea21_v_cd_87_16'
# Industry shifters
...
X = sm.add_constant(X, prepend=False)  # replicate often expects the constant present
...
metadata = {
    ...
    'comments': 'Econometrica 2022 — Table 1, Col (3), Panel A: Δln wage (2016–1980) on Panel-A task displacement, industry shifters, 1980 manufacturing share; FE: gender & education.'
}

# ---------- Run replicate ----------
replicate(
    metadata=metadata,
    y=yc,                          # log DV already constructed; DO NOT exponentiate
    X=Xc,
    interest=tdvar,                # variable of interest
    fe=fe_indices,                 # absorb gender & education FE
    elasticity=False,              # not an elasticity; coefficient is semi-elasticity wrt pp displacement
    weights=wc,                    # weights aligned to regression sample
