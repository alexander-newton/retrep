# Table 2 Column 5 — Anderson et al. (2020): State-level, log DV, OLS, pweights=births, cluster by state
import yaml
import os, glob, sys
import pandas as pd
...
# State-specific linear trends (NEW FOR COLUMN 5)
state_trends = [col for col in df.columns if col.startswith('state_FE_') and col.endswith('_trend')]

# Fixed effects (handled by replicate via fe=)
fe_vars = ['state_fips','year']

# Build X (treatment first for readability)
X = sm.add_constant(df[[treat] + policy_controls + demo_controls + env_proxies + state_trends + fe_vars], prepend=False)
...
metadata = {
    ...
    'comments': 'Replicates Table 2, column (5): demographics + policies + typhoid + nonpulmonary TB + state trends'
}
