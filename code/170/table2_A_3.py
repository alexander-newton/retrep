...
need = ['ln_inv', 'statenum', 'year', 'pop1940'] + controls + main_vars
df = df.dropna(subset=need).copy()

# -------------------------------
# Dependent variable: replicate() logs internally → pass exp(ln_inv)
# -------------------------------
y = np.exp(df['ln_inv'].to_numpy())
...
# -------------------------------
# Weights and two-way clustering
# -------------------------------
...
metadata = {
    'paper_id': '170',
    'table_id': '2',
    'panel_identifier': 'A_3',  # Panel A, Column (3)
    'model_type': 'log-log',
    'comments': 'Table 2 Panel A Col (3): ln_inv ~ ln(1 - mtr90_lag3) + ln(1 - top_corp_lag3) + rd_credit_lag3 + L.real_gdp_pc + L.population_density; state & year FE; [aw=pop1940]; two-way cluster.'
}
