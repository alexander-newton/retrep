...
# Keep 1939–2000 so L. controls exist for 1940; exclude states; build lags; then keep 1940–2000
# -------------------------------
...
# -------------------------------
# Transform tax variables to ln net-of-tax (ln(1 - tau)), tau in percent
# -------------------------------
def ln_net(series):
...
need = ['lncit', 'statenum', 'year', 'pop1940'] + controls + main_vars
df = df.dropna(subset=need).copy()

# -------------------------------
# Dependent variable: replicate() logs internally → pass exp(lncit)
# -------------------------------
y = np.exp(df['lncit'].to_numpy())
...
# -------------------------------
# Weights and two-way clustering (if supported by replicate())
# -------------------------------
...
metadata = {
    'paper_id': '170',
    'table_id': '2',
    'panel_identifier': 'A_2',  # Panel A, Column (2)
    'model_type': 'log-log',
    'comments': 'Table 2 Panel A Col (2): lncit ~ ln(1 - mtr90_lag3) + ln(1 - top_corp_lag3) + rd_credit_lag3 + L.real_gdp_pc + L.population_density; state & year FE; [aw=pop1940]; two-way cluster.'
}
...
replicate(
    ...
    kwargs_ols={
        ...
        'cov_kwds': {
            'groups': [
                df['statenum_fiveyear_gid'].to_numpy(),
                df['year'].astype(int).to_numpy()
            ]
        }
    },
